...
const PdfSign = () => {
  ...
  useEffect(() => {
    ...
    const start = (e) => {
      ...
      const pos = getPos(e);
      ctx.beginPath();
      ctx.strokeStyle = inkColor; // Uses the state color
      ctx.lineWidth = 2.5;
      ctx.lineCap = "round";
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      const finalX = sigPos.x * scaleX;
      const finalY = pdfHeight - (sigPos.y * scaleY) - (sigPos.height * scaleY);
            ...
            <aside className="bg-white/[0.03] p-8 rounded-[2.5rem] border border-white/10 space-y-8 sticky top-8 backdrop-blur-md">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4 items-center">
                  ...
                  {/* Custom Picker Restored */}
                  <input 
                    type="color" 
                    value={inkColor} 
                    onChange={(e) => { setInkColor(e.target.value); clearSignature(); }} 
                    className="w-10 h-10 bg-transparent border-none cursor-pointer p-0 overflow-hidden rounded-full" 
                  />
                </div>
              </div>
